      ...
      >
        ...
        <nav className="hidden items-center gap-1 text-sm md:flex">
          ...
          <details ref={articlesRef} open={openMenu === 'articles'} className="group relative">
            ...
            <div className="absolute right-0 top-10 min-w-48 rounded-xl border border-[var(--line-soft)] bg-[#f8f7f3]/95 p-2 shadow-lg backdrop-blur">
              ...
              <Link href="/archives" className="block rounded px-2.5 py-1.5 text-zinc-700 hover:bg-zinc-100" onClick={() => setOpenMenu(null)}>
                归档
              </Link>
              {tags.map((t) => (
                <Link
                  key={t.id}
                  href={`/category/${encodeURIComponent(t.slug)}`}
        ...
        <details className="group relative shrink-0 md:hidden">
          ...
          <nav className="absolute right-0 z-20 mt-2 min-w-44 rounded-lg border border-[var(--line-soft)] bg-[#f8f7f3] p-2 shadow-lg">
            ...
            <Link href="/archives" className="block rounded px-3 py-2 text-sm hover:bg-zinc-100">
              归档
            </Link>
            <p className="px-3 pt-2 text-xs text-zinc-500">分类</p>
            {tags.map((t) => (
              <Link key={t.id} href={`/category/${encodeURIComponent(t.slug)}`} className="block rounded px-3 py-2 text-sm hover:bg-zinc-100">
                #{t.name}
              </Link>
            ))}
